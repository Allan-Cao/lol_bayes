...
class BayesApiWrapper(object):
    ...
    def _load_credentials(self):
        if not os.path.isfile(self.credentials_file):
            ...
            with open(file=self.credentials_file, mode="w+", encoding="utf8") as f:
                json.dump(
                    {"username": email, "password": password}, f, ensure_ascii=False
                )
    ...
    def get_game_summary(self, game_rpgid):
        return self.get_game_asset(game_rpgid, "GAMH_SUMMARY")

    def get_game_details(self, game_rpgid):
        return self.get_game_asset(game_rpgid, "GAMH_DETAILS")
    ...
    def get_game_asset(self, game_rpgid, asset_name):
        asset_url = self._do_api_call(
            f"GET", f"emh/v1/games/{game_rpgid}/download", data={"type": asset_name}
        )["url"]
    ...
    def get_game_list(
        ...
        *,
        ...
        to_timestamp: Optional[Union[datetime, str, int, float]] = None,
